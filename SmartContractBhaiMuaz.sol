...
contract AMintPass is ERC721Enumerable {
    ...
    uint256 public PriceRate = 1 ether;  //matic
    uint256 public constant MAxWallet = 7;
    uint256 public  MintPeriodofTime;
    uint256 public constant mintPasses = 10;
    uint256 public mintPassesMinted;
    uint256 public mintingStartTime;
    bool public mintingEnabled = true;
    bool private WHITELISTED_Enabled = false;
    
    mapping(address => bool) public whitelist;
    address private Address_fee = 0x617F2E2fD72FD9D5503197092aC168c91465E7f2;
    string private TokenURI = "https://gateway.pinata.cloud/ipfs/QmToF92ZGLihXcjiHb3GXPdEPV9NnMkkAUhArximxW8Jh2/";
    ...
    function mint1Ether(uint256 _numToMint) public payable{
    
         require(mintingEnabled == false, "Minting Not Enabled");
        //    require(mintingStartTime > 0, "Minting has not started yet");
         require(block.timestamp < mintingStartTime + 7 days, "Minting period has ended");
        if (WHITELISTED_Enabled) {
        ...
        if(msg.sender == owner()) {
         ...
         if (mintCounts[msg.sender] == 0) {
        _mintAddresses.push(msg.sender);
    }
      mintCounts[msg.sender] += 1;
} 

    function getMintedTokensByWallet(address _wallet)external view returns (uint256[] memory)
    {
...
// permision function on and off 
    function MintingOnAngOff() public onlyOwner   {
        if (mintingEnabled) {
  ...
  function MintData() public view returns (address[] memory, uint256[] memory) {
    ...
    for (uint256 i = 0; i < _mintAddresses.length; i++) {
        mintAmounts[i] = mintCounts[_mintAddresses[i]];
    }
